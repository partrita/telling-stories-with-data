...
**타당도(Validity)**있는 측정이란 우리가 측정하는 값이 실제 알고자 하는 연구 질문(추정량)과 밀접하게 연관되어 있음을 뜻합니다.\index{measurement!valid} 이는 "우리가 정말로 재야 할 것을 재고 있는가?"에 관한 적절성의 문제입니다. @sec-on-writing에서 다룬 **추정량(Estimand)**\index{estimand}은 흡연이 기대 수명에 미치는 실제 영향과 같은 '객관적 실체'입니다. 따라서 우리는 단순히 흡연에 대한 개인의 주관적 견해가 아니라, 그가 피운 담배 개수와 실제 수명과 같은 관련 정보를 정확히 측정해야 합니다.
