...
- *여기서 일어난 일은 바람이 바뀌었다는 것입니다* 읽기(원제: *What has happened down here is the winds have changed*), [@Gelman2016]
  - 복제 위기(replication crisis)와 사회 과학이 이에 대응하여 어떻게 변화했는지에 대한 개요를 제공하는 블로그 게시물입니다.
- *과학적 컴퓨팅에서의 충분히 좋은 관행* 읽기(원제: *Good enough practices in scientific computing*), [@wilsongoodenough]
  - 컴퓨터 사용 방식에 초점을 맞춰 데이터 과학을 수행하는 방법에 대한 명확하고 쉽게 채택할 수 있는 권장 사항을 제공하는 논문입니다.
- *미래의 자신과의 관계를 개선하는 방법* 읽기(원제: *How to improve your relationship with your future self*), [@bowers2016improve]
  - 분석에 초점을 맞춰 데이터 과학을 수행하는 방법에 대한 명확하고 쉽게 채택할 수 있는 권장 사항을 제공하는 논문입니다.
- *코드 공유 장벽 극복하기* 시청(원제: *Overcoming barriers to sharing code*), [@monicatalks]
  - 이 비디오는 코드 공유에 익숙해지는 것에 대한 개인적인 성찰입니다.
- *reprex를 만들어주세요... 제발* 시청(원제: *Make a reprex... Please*), [@sharlatalks]
  - 이 비디오는 도움을 요청할 때 재현 가능한 예제를 만드는 것이 왜 그렇게 중요한지 자세히 설명합니다.
- *tidyverse 스타일 가이드* 읽기(원제: *The tidyverse style guide*), [@tidyversestyleguide]
  - R에서 코딩할 때 권장되는 모범 사례를 문서화한 웹사이트입니다.
- *코드 냄새와 느낌* 시청(원제: *Code smells and feels*), [@codesmells]
