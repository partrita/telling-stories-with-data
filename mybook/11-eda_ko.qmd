...
- *데이터 분석의 미래* 읽기(원제: *The Future of Data Analysis*), [@tukey1962future]
  - 20세기 통계학자 존 투키는 통계학에 많은 기여를 했습니다. 이 논문에서 "일반적인 고려 사항" 1부에 집중하세요. 이는 데이터를 통해 무언가를 배워야 하는 방식에 대해 시대를 앞서갔습니다.
- *데이터 정리의 모범 사례* 읽기(원제: *Best Practices in Data Cleaning*), [@bestpracticesindatacleaning]
...
- *전체 게임* 시청(원제: *The Whole Game*), [@hadleycodes]
  - 자체 포함된 EDA 작업 예시를 제공하는 비디오입니다. 좋은 점 중 하나는 전문가가 실수를 하고 그것을 고치는 것을 볼 수 있다는 것입니다.

**핵심 개념 및 기술**

- 탐색적 데이터 분석(EDA)은 데이터를 면밀히 살펴보고 그래프, 표, 모델을 구성하며 새로운 데이터셋과 친숙해지는 과정입니다. 우리는 크게 세 가지 측면을 이해하고자 합니다.
  1. 개별 변수 자체의 특성
  2. 다른 변수와의 관계 속에서 나타나는 각 변수의 의미
  3. 존재하지 않는 데이터(결측치)의 맥락
- EDA 과정을 통해 데이터셋의 잠재적인 문제와 특징을 파악하며, 이것이 이후 분석 결정에 어떤 영향을 미칠지 이해합니다. 특히 결측치(Missing values)와 이상치(Outliers)에 대해 주의 깊게 살펴봅니다.
...
EDA는 그 자체로 목적이라기보다 목표를 향해 나아가는 과정에 가깝습니다.\index{exploratory data analysis!how to} EDA 결과는 최종 논문의 데이터 섹션 등을 구성하는 밑바탕이 되지만, 분석 과정에서 만든 모든 결과물이 논문에 포함되지는 않습니다. 효과적인 EDA를 위해서는 별도의 Quarto 문서를 만들어 코드를 실행하고 그 자리에서 떠오른 생각들을 메모해두는 것이 좋습니다. 이전 코드를 지우지 않고 계속 덧붙여 나가다 보면 데이터 탐색 과정을 고스란히 담은 귀중한 기록(Notebook)이 됩니다. 이 문서는 이후 수행될 정밀 분석과 모델링의 든든한 가이드라인이 될 것입니다.

EDA는 다양한 기법을 활용하며 분석가가 선택할 수 있는 도구 또한 무궁무진합니다 [@staniak2019landscape]. 시각화 도구뿐만 아니라 모든 가능한 수단을 동원해야 합니다. 데이터를 직접 훑어보기도 하고, 표와 그래프를 그리며, 요약 통계량을 계산하고, 필요하다면 간단한 모델을 만들어볼 수도 있습니다. 핵심은 세밀하게 완벽을 기하기보다 빠르게 반복하며 데이터를 입체적으로 이해하는 것입니다. 흥미로운 점은, 현재 가지고 있는 데이터를 철저히 이해하는 과정이 때로는 우리가 미처 확보하지 못한 데이터가 무엇인지 깨닫게 해준다는 사실입니다.

우리는 특히 다음 세 가지 질문에 집중합니다.\index{exploratory data analysis!aims}

- 개별 변수의 분포 및 속성을 이해합니다.
- 변수 간의 관계를 이해합니다.
- 존재하지 않는 것을 이해합니다.

EDA를 수행하고 완료하는 데 필요한 올바른 프로세스나 단계는 없습니다. 대신, 관련 단계와 도구는 데이터와 관심 있는 질문에 따라 달라집니다. 따라서 이 장에서는 미국 주 인구, 토론토 지하철 지연, 런던 에어비앤비 목록을 포함한 다양한 EDA 예시를 통해 EDA 접근 방식을 설명합니다. 또한 @sec-farm-data 기반으로 결측 데이터로 돌아갑니다.
...
가장 적절한 결측치 처리 방식은 데이터의 성격과 분석 목적에 따라 다릅니다. 우리는 시뮬레이션을 통해 특정 선택이 결과에 어떤 함의를 가지는지 면밀히 검토해야 합니다. 크게 두 가지 방향이 있습니다. 결측치가 포함된 관측치를 아예 제거하거나, 적절한 값으로 '대체(Imputation)'하는 것입니다. (모델링 차원에서의 솔루션도 있으나 이는 본 장의 범위를 벗어납니다.) 어떤 방식을 택하든 그 한계를 인지하고 신중하게 전달해야 합니다.
...
@tbl-imputationoptions 이러한 접근 방식 중 어느 것도 순진하게 적용되어서는 안 된다는 것을 분명히 합니다. 예를 들어, 플로리다의 인구는 8,277명이어야 합니다. 모든 주에 걸쳐 평균을 대체하면 4,308명으로 추정되고, 다중 대체는 11,197명으로 추정됩니다. 전자는 너무 낮고 후자는 너무 높습니다. 만약 대체가 답이라면, 다른 질문을 찾는 것이 더 나을 수 있습니다. 이것이 개인 정보의 공개를 제한하는 특정 상황을 위해 개발되었다는 점을 지적할 가치가 있습니다[@myboynick].
...
마지막으로, 더 평범하지만 마찬가지로 중요한 것은 때때로 결측 데이터가 특정 값으로 변수에 인코딩된다는 것입니다.\index{missing data!encoding} 예를 들어, R에는 "NA" 옵션이 있지만, 때로는 결측된 경우 숫자 데이터가 "-99" 또는 "9999999"와 같은 매우 큰 정수로 입력됩니다. @sec-hunt-data 소개된 Nationscape 설문 조사 데이터셋의 경우, 세 가지 유형의 알려진 결측 데이터가 있습니다.
...
EDA를 수행할 때 정해진 단 하나의 정답은 없지만, 우리는 보통 다음과 같은 질문을 던집니다.

- **데이터의 형태**: 각 변수의 자료형(Class)은 무엇인가? 값의 범위와 분포는 예상과 일치하는가?
- **의외의 발견**: 이상치(Outliers)처럼 튀는 데이터는 무엇인가? 반대로 당연히 있어야 하는데 누락된(Missing) 데이터는 무엇인가?
- **분석 목표 구체화**: 예를 들어, 특정 역이나 시간대가 지연 발생과 어떤 상관관계가 있는지 파악하는 것을 목표로 삼을 수 있습니다.

분석을 진행하면서 발견한 모든 특이 사항과 가정을 꼼꼼히 문서화하는 것이 중요합니다. 이는 훗날 모델링 단계에서 의사결정의 근거가 되기 때문입니다. 자연과학 분야에서 '연구 노트(Lab notebook)'가 법적 효력을 갖는 중요한 문서가 되듯, 데이터 과학에서도 이 기록은 분석의 재현성과 신뢰성을 담보하는 핵심 자산이 됩니다 [@nihtalk].
...
원본 데이터셋은 우리의 것이 아니므로 서면 허가를 받지 않고는 공개해서는 안 됩니다. 예를 들어, 입력 폴더에 추가할 수 있지만, @sec-reproducible-workflows 다룬 ".gitignore" 항목을 사용하여 GitHub에 푸시하지 않도록 할 수 있습니다. `read_csv()`의 "guess_max" 옵션은 열 유형을 지정할 필요가 없도록 도와줍니다. 일반적으로 `read_csv()`는 처음 몇 행을 기반으로 열 유형을 추측합니다. 그러나 때로는 처음 몇 행이 오해의 소지가 있으므로 "guess_max"는 더 많은 행을 살펴보고 무슨 일이 일어나고 있는지 파악하도록 강제합니다. Inside Airbnb에서 복사한 URL을 URL 부분에 붙여넣으십시오. 다운로드되면 로컬 사본을 저장하십시오.
...
@fig-airbnbpricesbunch 를 보면, 1,000달러 미만의 가격대에 데이터가 집중되어 있으며 대부분의 숙소가 1박당 250달러 미만임을 알 수 있습니다(@fig-airbnbpricesbunch-1). @sec-clean-and-prepare 의 연령 데이터에서 특정 숫자에 값이 몰리는 현상을 보았듯이, 가격 데이터에서도 0이나 9로 끝나는 특정 가격대에 숙소들이 밀집해 있는 '뭉침(Bunching)' 현상을 발견할 수 있습니다. 90달러에서 210달러 사이를 확대하고 빈(Bin)의 크기를 1로 설정해 더 자세히 살펴보겠습니다(@fig-airbnbpricesbunch-2).
