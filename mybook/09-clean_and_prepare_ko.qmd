...
- *데이터 페미니즘*(원제: *Data Feminism*), 캐서린 디냐지오 & 로런 클라인 [@datafeminism2020] 읽기.
  - 데이터 정제 과정에서 내리는 수많은 결정이 어떤 사회적, 정치적 함의를 갖는지 논의하는 5장 "유니콘, 관리인, 닌자, 마법사, 록 스타"에 주목하세요.
- *데이터 과학을 위한 R*(원제: *R for Data Science*), 해들리 위컴 & 개릿 그롤먼드 [@r4ds] 읽기.
  - 정돈된 데이터(Tidy data)의 개념과 이를 만들기 위한 전략을 설명하는 6장 "데이터 정리"를 참고하세요.
- *R을 이용한 데이터 정제 입문*(원제: *An Introduction to Data Cleaning with R*), 에드윈 드 용 & 마크 반 데르 루 [@de2013introduction] 읽기.
  - 원시 데이터를 기술적으로 올바른(Technically correct) 데이터로 변환하는 과정을 상세히 다루는 2장을 읽어보세요.
- *워싱턴 포스트 선거 공학 팀의 교훈*(원제: *What the Washington Post’s election engineering team had to learn about election data*), 제레미 바워스 [@washingtonpostelections] 읽기.
  - 실제 대규모 선거 데이터를 다루면서 마주하게 되는 현실적인 문제들을 다룹니다.
- *계약으로서의 열 이름*(원제: *Column Names as Contracts*), 에밀리 리더러 [@columnnamesascontracts] 읽기.
  - 변수 이름을 일정한 규칙에 따라 명명하는 것이 협업과 유지보수에 어떤 이점을 주는지 소개합니다.
- *역사적 해수면 온도 기록의 개선*(원제: *Combining statistical, physical, and historical evidence to improve historical sea-surface temperature records*), 듀오 찬 외 [@Chan2021Combining] 읽기.
  - 서로 다른 시기에 각기 다른 선박에서 수집된 방대한 데이터를 하나의 일관된 데이터셋으로 통합하는 과정의 어려움을 잘 보여줍니다.

**핵심 개념 및 기술**

- 데이터셋을 정제하고 준비하는 과정은 무수히 많은 결정을 수반하는 까다로운 작업입니다. 최종 결과물에 대한 명확한 계획을 세우고, 우리가 얻고자 하는 데이터의 형태를 미리 **시뮬레이션**해 보는 것이 무엇보다 중요합니다.
...
>
> — 린든 존슨 전 미국 대통령이 새로 임명된 내각 인사들의 천재성을 칭송하자, 샘 레이번 하원의장이 남긴 말(데이비드 핼버스탬의 *The Best and the Brightest* [@halberstam, p. 41]에서 재인용).

이 장에서는 데이터 정제 및 준비(Data cleaning and preparation)\index{data!cleaning}를 위한 체계적이고 공식적인 접근 방식을 제시합니다. 이 방식은 다음 세 가지 축을 중심으로 전개됩니다.
...
@sec-on-writing 장에서 말했듯이, 린든 존슨의 전기 작가 로버트 카로는 미국 36대 대통령과 관련된 모든 사람을 추적하는 데 수년을 보냈습니다. 카로와 그의 아내 이나는 존슨이 어디 출신인지 더 잘 이해하기 위해 텍사스 힐 컨트리에서 3년 동안 살았습니다. 카로는 존슨이 상원 의원으로서 D.C.에 머물던 곳에서 상원까지 달려갔다는 소식을 듣고, 존슨이 왜 달렸는지 이해하기 위해 그 경로를 여러 번 직접 달렸습니다. 카로는 존슨이 그랬던 것처럼 해가 뜰 때 그 경로를 달렸을 때 비로소 이해했습니다. 즉, 해가 상원 로툰다에 특히 영감을 주는 방식으로 비친다는 것을 알게 되었죠 [@caroonworking, p. 156]. 이러한 배경 작업은 그가 다른 누구도 알지 못했던 측면을 밝혀낼 수 있게 했습니다. 예를 들어, 존슨은 거의 확실히 첫 선거에서 승리를 훔쳤습니다 [@caroonworking, p. 116]. 우리는 이와 동일한 정도로 데이터를 이해해야 합니다. 우리는 비유적으로 모든 페이지를 넘겨보고 싶습니다.

음의 공간(Negative space) 개념은 디자인에서 잘 확립되어 있습니다. 이는 주제를 둘러싸고 있는 배경을 의미하죠. 때로는 음의 공간이 시각적 효과로 사용되기도 합니다. 예를 들어, 미국 물류 기업 FedEx의 로고는 E와 X 사이의 음의 공간에 화살표가 숨어 있습니다. 이와 유사하게, 우리는 우리가 가지고 있는 데이터와 가지고 있지 않은 데이터에 대해 명확히 인식해야 합니다 [@citemyboy]. 우리는 우리가 가지고 있지 않은 데이터가 어떤 식으로든 의미를 가지고 있으며, 잠재적으로 우리의 결론을 바꿀 정도로 중요할 수 있다는 사실을 늘 경계해야 합니다. 데이터를 정리할 때, 우리는 이상 징후를 찾습니다. 우리는 데이터셋에 있어야 하지만 없어야 하는 값뿐만 아니라 그 반대 상황, 즉 데이터셋에 있어야 하지만 없는 값에도 관심이 있습니다. 이러한 상황을 식별하는 데 사용하는 세 가지 도구는 그래프, 개수, 그리고 테스트입니다.

우리는 또한 이러한 도구를 사용하여 올바른 관측치를 잘못된 것으로 변경하지 않도록 주의해야 합니다. 특히 정리 및 준비에 여러 단계가 필요한 경우, 한 단계에서 수정된 내용이 나중에 취소될 수도 있습니다. 우리는 그래프, 개수, 특히 테스트를 사용하여 이를 방지합니다. 이러한 도구들의 중요성은 데이터셋의 크기가 커질수록 기하급수적으로 증가합니다. 작고 중간 규모의 데이터셋은 수동 검사나 분석가의 판단에 더 적합할 수 있지만, 대규모 데이터셋은 필연적으로 더 효율적이고 자동화된 전략을 필요로 합니다 [@hand2018statistical].

### 그래프

그래프는 데이터를 정리할 때 귀중한 도구입니다. 왜냐하면 데이터셋의 각 관측치를 잠재적으로 다른 관측치와 관련하여 보여주기 때문이죠\index{data!graphs}. 값이 속하지 않는 경우를 식별하는 데 매우 유용합니다. 예를 들어, 값이 숫자여야 하지만 여전히 문자라면 플롯되지 않고 경고가 표시됩니다. 그래프는 숫자 데이터에 특히 유용하지만, 텍스트나 범주형 데이터에도 여전히 유용합니다. 예를 들어, 청소년 설문 조사에서 사람의 나이에 관심이 있는 상황을 가정해 봅시다. 다음과 같은 데이터가 있습니다.
...
#| fig-cap: "시뮬레이션된 청소년 설문 조사 데이터셋의 연령 분포를 통해 데이터 문제를 식별합니다."
...
@fig-youth-survey-1 은 예상치 못한 값 150을 보여줍니다. 가장 가능성 있는 설명은 데이터 입력 시 소수점을 놓쳤으며, 실제로는 15.0이어야 한다는 것이죠. 이를 수정하고 문서화한 다음 그래프를 다시 그리면 모든 것이 더 타당해 보일 것입니다 (@fig-youth-survey-2).

### 개수

우리는 대부분의 데이터를 올바르게 얻는 데 집중하고 싶으므로 고유 값의 개수에 관심이 있습니다\index{data!counts}. 대부분의 데이터가 가장 일반적인 값들에 집중되기를 기대하죠. 하지만 반대로 특히 흔하지 않은 것이 무엇인지 살펴보는 것도 매우 유용합니다. 이러한 예외들을 얼마나 다룰지는 분석의 필요에 따라 달라집니다. 궁극적으로, 우리가 하나를 수정할 때마다 얻게 되는 추가 관측치는 매우 적습니다. 심지어 단 하나일 수도 있죠. 하지만 개수 확인은 텍스트나 범주형 데이터에서 특히 유용하며, 때로는 숫자 데이터에서도 큰 도움을 줍니다.

각각 "호주"를 의미하는 텍스트 데이터의 사례를 살펴보겠습니다\index{text!cleaning}.
...
이 개수를 확인하면 귀한 시간을 어디에 투자해야 할지 알 수 있습니다. "Australie"를 "Australia"로 단 하나만 변경해도 사용 가능한 데이터 양이 거의 두 배가 되기 때문이죠.

숫자 데이터로 잠시 돌아가면, @Preece1981 은 변수의 각 관측치에서 '마지막 숫자'의 개수를 플로팅해 볼 것을 권장합니다. 예를 들어, 변수의 관측치가 "41.2", "80.3", "20.7", "1.2", "46.5", "96.2", "32.7", "44.3", "5.1", "49.0"이라면, 마지막 숫자로 0, 1, 5는 한 번씩, 3, 7은 두 번씩, 2는 세 번 나타납니다. 우리는 대개 이러한 마지막 숫자가 균일하게 분포되어야 한다고 예상합니다. 하지만 놀랍게도 그렇지 않은 경우가 많으며, 그 불균형의 양상은 매우 유익한 정보를 제공합니다. 예를 들어, 데이터가 인위적으로 반올림되었거나 측정 기기 혹은 수집자에 따라 편향이 있었을 가능성을 시사하기 때문입니다.

예를 들어, 이 장의 뒷부분에서 2019년 케냐 인구 조사 데이터를 수집하고 정리할 예정입니다. 여기서는 해당 데이터셋을 미리 활용해 연령의 마지막 숫자 분포를 살펴보겠습니다. 즉, 35세에서는 "5"를, 74세에서는 "4"를 추출하는 식이죠. @tbl-countofages 는 일부 응답자가 연령을 가장 가까운 5단위나 10단위로 응답하기 때문에 발생하는 '연령 집중(Age heaping)' 현상을 보여줍니다. 만약 그러한 패턴이 없는 연령 변수가 있다면, 이는 단순한 응답이 아닌 다른 정교한 방식(예: 생년월일 기반 계산)으로 생성된 데이터라고 짐작할 수 있습니다.
...
#| tbl-cap: "2019년 케냐 인구 조사 나이로비 지역 데이터의 마지막 숫자 분포: 0과 5의 뚜렷한 집중 현상"
...
arrow::read_parquet(
  ...
  setNames(c("나이의 마지막 숫자", "총 횟수"))

```

### 테스트

@sec-reproducible-workflows 장에서 언급했듯이, 코드를 작성한다고 해서 모두가 다 같은 '프로그래머'인 것은 아닙니다. 취미로 간단한 코드를 짜는 사람과 제임스 웹 우주 망원경을 작동시키는 전문적인 코드를 짜는 사람 사이에는 분명한 격차가 존재하죠. @weinbergpsychology [p. 122]는 아마추어와 전문가를 가르는 중요한 기준 중 하나로 '후속 사용자(Subsequent user)의 존재 여부'를 꼽았습니다. 입문자 시절에는 오직 자신만이 결과물을 확인하는 코드를 작성합니다. 과제 제출용 코드가 대표적인 예입니다. 하지만 전문가는 다른 사람들과 협업하거나, 훗날 자신의 코드를 다시 보게 될 누군가를 위해 코드를 작성합니다.

오늘날 대다수의 학술 연구는 코드에 기반하고 있습니다. 연구 결과가 지속적인 지식으로서 가치를 지니려면, 그 바탕이 되는 코드는 다른 연구자가 쉽게 이해하고 재현할 수 있어야 합니다. 전문가는 자신의 코드가 타인에게 읽힐 것을 고려하여 적절한 주의를 기울이는데, 그 핵심이 바로 **테스트(Testing)**입니다\index{data!tests}.
...
어떤 조건들은 데이터셋에 반드시 포함되거나 준수되어야 합니다. 이는 연구자의 경험, 도메인 지식, 또는 앞서 수행한 시뮬레이션 단계에서 도출되죠. 예를 들어 나이 변수에는 음수가 올 수 없으며, 110세를 넘는 경우는 극히 드물 것입니다. 이러한 상식적인 조건들을 테스트로 명시해야 합니다.

구체적인 사례로 케냐\index{Kenya!Nairobi}의 인구 상위 5대 카운티(나이로비, 키암부, 나쿠루, 카카메가, 분고마)를 분석한다고 가정해 봅시다.
...
위의 결과에서 알 수 있듯이, 명백한 오타(숫자)를 수정했음에도 여전히 '키암브루'나 '카바메가' 같은 오타가 남아 있어 테스트를 통과하지 못했습니다. 우리는 이 테스트를 모두 통과할 때까지 정제 과정을 끈기 있게 반복해야 합니다.
...
-   **이름 및 문자열(Names)**: 이름은 문자형이어야 합니다. 맥락에 따라 숫자나 특수 문자가 포함되지 않아야 한다는 규칙을 적용할 수도 있습니다.
-   **관측치 개수(Observation count)**: 데이터 정제 과정에서 의도치 않게 행이 삭제되거나 중복되는 일은 매우 흔하게 발생합니다. 조인(Join)이나 필터링 전후에 행의 개수를 확인하여 의도한 대로 데이터 규모가 유지되고 있는지 반드시 체크해야 합니다.
...
결과를 보면 우리가 설정한 규칙들 중 일부에서 오류(Fail)가 발생했음을 수치로 명확히 확인할 수 있습니다. @datavalidationbook 에는 이처럼 실무에서 곧장 활용할 수 있는 수많은 테스트 사례가 담겨 있습니다.

@sec-farm-data 장에서 언급했듯이 '성별' 변수는 특히 주의가 필요합니다. "남성"이나 "여성"이 아닌 소수 응답자가 존재할 수 있는데, 데이터 양이 너무 적어 분석에서 제외해야 할 상황이라 하더라도 이를 단순히 무시해서는 안 됩니다. 응답자에 대한 존중을 담아, 분석 제외 사유와 함께 그들의 다른 특성들이 전체 데이터와 어떻게 유사하거나 다른지 부록 등에서 간략히 논의하는 것이 권장됩니다.

#### 데이터 타입(클래스)

:::{.content-visible when-format="pdf"}
"미국인은 돈에 집착하고, 영국인은 계급(Class)에 집착한다"는 농담이 있습니다. 데이터 정제 과정에서만큼은 우리도 '데이터 타입(Class)'에 집착하는 영국인이 되어야 합니다\index{data!class}. 데이터 타입은 분석의 성패를 가르는 결정적인 요소입니다. ["R 필수 사항" 온라인 부록](https://tellingstorieswithdata.com/20-r_essentials.html)에서 다루었듯이, 여기서는 "숫자형(numeric)", "문자형(character)", "요인형(factor)"에 집중하겠습니다. 변수에 잘못된 클래스를 할당하면 이후의 모든 통계 분석 결과가 심각하게 왜곡될 수 있습니다.
:::
...
@sec-its-just-a-linear-model 장에서 자세히 다룰 로지스틱 회귀 분석을 적용해 보겠습니다. 하나는 그룹 정보를 정수형 변수로, 다른 하나는 요인형 변수로 사용하여 모델을 만들었습니다. @tbl-effect-of-class 를 보면 두 모델의 결과가 확연히 다름을 알 수 있습니다. 정수형 변수는 그룹 간에 선형적인 순서 관계가 있다고 가정하는 반면, 요인형 변수는 각 그룹을 독립적인 범주로 취급하여 분석에 훨씬 더 많은 유연성을 부여하기 때문입니다.

```{r}
#| label: tbl-effect-of-class
#| tbl-cap: "데이터 타입(클래스)의 차이가 회귀 분석 결과에 미치는 영향"
...
이처럼 데이터 타입은 모델링에 결정적인 영향을 미치므로, 분석 전 반드시 엄격한 테스트 절차를 거치는 것이 좋습니다\index{data!class}. 미국의 유명 퀀트 투자 회사인 제인 스트리트(Jane Street)가 특정 프로그래밍 언어인 OCaml을 고집하는 이유 중 하나도 바로 데이터 타입을 철저하게 다루는 시스템을 통해 치명적인 오류를 미연에 방지할 수 있기 때문입니다 [@somers2015]\index{Jane Street}. 데이터가 비즈니스나 정책 결정의 핵심일수록 데이터 타입의 중요성은 아무리 강조해도 지나치지 않습니다.
...
따라서 우리는 데이터를 불러오는 단계부터 **의도적으로** 데이터 타입을 지정하는 습관을 가져야 합니다. `read_csv()` 함수가 자동으로 타입을 추측하게 내버려 두는 대신, `col_types` 인자를 사용하여 각 열의 성격을 명확히 규정하는 것을 강력히 권장합니다.
...
이것은 일반적으로 데이터셋을 처음 읽어들여 구조를 빠르게 파악한 뒤, 필요한 열과 클래스만 지정하여 다시 정교하게 읽어들이는 반복적인 과정입니다. 비록 약간의 추가 작업이 필요하지만, 데이터의 성격을 명확히 규정하는 것은 분석의 신뢰성을 위한 필수 과정입니다.

#### 날짜와 시간

어떤 데이터 과학자가 날짜 데이터와 씨름해 본 적이 있는지 알아보는 가장 쉬운 방법은, 그에게 "날짜와 함께 작업하는 게 어떤가요?"라고 물어보는 것입니다\index{data!cleaning}. 그가 즉시 고개를 절레절레 흔들며 끔찍했던 오해와 버그에 대한 무용담을 쏟아낸다면, 그는 산전수전 다 겪은 베테랑일 가능성이 매우 높습니다!

날짜 데이터는 꼼꼼한 검증이 필수적입니다. 데이터 과학계의 국제 표준인 **ISO 8601**에 따라, 날짜는 가급적 **YYYY-MM-DD** 형식을 유지해야 합니다. 7월 1일을 "2022-07-01"로 쓸지 "01 July 2022"로 쓸지에 대해서는 개인의 취향이 갈릴 수 있지만, 컴퓨터가 오차 없이 인식하고 정확히 정렬하기 위해서는 국제 표준 형식을 사용하는 것이 가장 좋습니다.

날짜 검증을 위해 다음과 같은 테스트를 수행해 보세요.

-   **구성 요소 확인**: 요일 변수가 있다면 월, 화, ..., 일요일 외에 다른 부적절한 값이 있는지 확인합니다. 또한 일주일 7일 혹은 1년 12개월 데이터가 모두 빠짐없이 존재하는지 체크합니다.
-   **월별 일수 확인**: 각 월의 일수가 실제 달력과 일치하는지 확인합니다. 예를 들어 9월은 30일까지 있고, 2월은 28일(윤년에는 29일)까지 있어야 합니다.
-   **순서 확인**: 데이터가 시간 순서대로 바르게 정렬되어 있는지 확인합니다. 반드시 시간순일 필요는 없으나, 흐름이 끊기거나 과거로 역행하는 부분이 있다면 데이터 수집 과정의 오류를 강력히 의심해 봐야 합니다.
-   **연도 범위 확인**: 모든 연도 값이 분석 대상 기간(예: 2010~2020년) 내에 있는지 확인합니다.

@sec-fire-hose 장에서 `opendatatoronto` 패키지를 사용하여 2021년 토론토 쉼터 사용량 데이터셋을 살펴보았습니다\index{Canada!Toronto shelter usage}. 이번에는 날짜와 관련된 더 복잡한 문제들을 다루기 위해 2017년도 데이터를 검토해 보겠습니다. 먼저 데이터를 불러옵니다.^[토론토 시 오픈 데이터 포털의 주소가 변경되었을 수 있습니다. 아래 코드가 작동하지 않는다면 `toronto_shelters_2017 <- read_csv("https://www.tellingstorieswithdata.com/inputs/data/earlier_toronto_shelters.csv")`를 직접 실행하세요.]
...
데이터를 불러와 구조를 살펴보면, `OCCUPANCY_DATE` 열이 날짜가 아닌 문자열(character) 타입으로 잘못 인식되어 있음을 알 수 있습니다.
...
이처럼 날짜가 "17-01-01"과 같은 문자열로 되어 있으면, 이를 실제 날짜 객체로 변환해야 합니다. 이를 위해 `lubridate` 패키지가 유용하게 쓰이지만, 그전에 먼저 데이터가 정확히 어떤 형식인지 파악해야 합니다. 이 데이터에서는 연-월-일 순서로 되어 있는 듯하므로 `lubridate::ymd()`를 시도해 볼 수 있습니다.
...
이처럼 날짜 데이터는 변환 후 반드시 **합리성 검토(Reasonableness check)**를 거쳐야 합니다. "2017년 쉼터 사용량" 데이터셋에 2020년 데이터가 포함되어 있다면, 이는 원본 데이터 자체에 오류가 있거나 정제 로직에 심각한 문제가 있음을 의미하기 때문입니다.
...
날짜 데이터의 일자(Day) 분포를 그려보면, 무언가 체계적인 문제가 있음을 직감할 수 있습니다 (@fig-homeless-daycount-1). 만약 날짜가 정상적으로 기록되었다면 1일부터 31일까지 골고루 나타나야 하지만, 데이터가 특정 구간에 쏠려 있는 것을 볼 수 있죠.
...
@fig-homeless-daycount-2 에서 보듯 데이터 순서와 날짜가 일치하지 않는 것은, 엑셀 등에서 편집하는 과정 중에 날짜 형식이 뒤섞였음을 시사합니다. 상식적으로 2017년 데이터에 2020년이 섞여 있거나 날짜가 과거로 역행할 리 없기 때문입니다.

가장 의심되는 원인은 "연-월-일(YMD)"과 "연-일-월(YDM)" 형식이 혼용된 것입니다. 특히 월과 일이 모두 12 이하인 경우 두 형식을 구분하기가 매우 어렵습니다. 모니카 알렉산더(Monica Alexander)가 제안한 대로, 오류가 의심되는 연초 12일 구간에 대해 형식을 뒤집어 적용해 보겠습니다.
...
여전히 모든 문제가 해결된 것은 아니지만, 정정 전과 비교하면 데이터의 흐름이 훨씬 더 자연스러워졌고 매월 초반에 쏠려 있던 현상도 상당 부분 완화되었습니다. 실제 데이터 정제는 이처럼 가설을 세우고, 시각화로 확인하며, 끊임없이 보완해 나가는 끈질긴 과정입니다.

#### 문자열(Strings)

문자열 정제는 정규 표현식(Regular expressions)\index{regular expressions}과 `stringr` 패키지를 활용한 집요한 싸움입니다. 

문자열 정제의 대표적인 사례로 **"검거된 오랑우탄(Captured Orangutans)"** 연구를 들 수 있습니다. 연구자들이 야생 오랑우탄의 서식지 데이터를 정리하던 중, 특정 지역에서만 인구 밀도가 비정상적으로 높게 나타나는 것을 발견했습니다. 원인을 파악해 보니, 엑셀에서 숫자 "1"이 알파벳 대문자 "I"로 잘못 입력되어 있었고, 시스템은 이를 숫자로 인식하지 못해 데이터가 누락되거나 왜곡되었던 것이죠. 이처럼 눈으로 보기에는 비슷해 보여도 컴퓨터에게는 완전히 다른 문자열을 찾아내는 것이 정제의 핵심입니다.

문자열 정제에서 마주치는 흔한 문제와 해결책은 다음과 같습니다.

-   **공백 문제**: "Toronto "와 "Toronto"는 서로 다른 데이터입니다. `str_trim()`으로 양 끝 공백을 제거하고, `str_squish()`로 내부의 이중 공백을 정리하세요.
...
@sec-its-just-a-linear-model 장에서 다시 다룰 구체적인 예시를 위해, 5km 달리기 기록과 마라톤(42.195km) 완주 기록 사이의 관계를 시뮬레이션해 보겠습니다 (@fig-fivekmvsmarathon-1). 
...
산점도는 우리가 지정한 선형 관계를 명확히 보여줍니다. 이제 이 데이터를 바탕으로 기본적인 데이터 가공 기법들을 익혀보겠습니다.
...
우리는 시뮬레이션을 사용하여 실제 데이터가 충족하기를 원하는 다양한 테스트를 설정할 수 있습니다. 예를 들어, 5km 및 마라톤 달리기 시간의 클래스가 숫자이기를 원합니다. 그리고 200개의 관측치를 원하죠.
...
5km 달리기 시간의 경우 15분 미만이거나 30분 이상인 값은 후속 조치가 필요한 이상치일 가능성이 높다는 것을 알고 있습니다.
...
>
> --- 해럴드 아벨슨(Harold Abelson) [@abelson1996structure]
...
또한 의미 없는 약어보다는 명확한 단어를 사용하는 것이 좋습니다. "f"보다는 "flag", "age_r"보다는 "age_respondent"가 훨씬 친절한 이름입니다. 연구에 따르면 너무 짧은 이름은 오히려 코드를 이해하는 시간을 늘린다고 하죠 [[@shorternamestakelonger]].
...
R에서는 열 이름의 일부분만 써도 데이터를 찾아주는 '부분 일치(Partial matching)' 기능이 있지만, 실무에서는 절대 사용하지 말아야 합니다. 코드가 모호해지고 나중에 버그를 찾기가 매우 힘들어지기 때문입니다.
...
우리는 연령대, 성별 및 도시/농촌 분포에 관심이 있습니다. 빠른 스케치는 @fig-tanzaniasketch 와 같을 수 있습니다.

![관심 있는 데이터셋의 빠른 스케치](figures/09-tanzania_sketch.png){#fig-tanzaniasketch width=70% fig-align="center"}
...
이 시뮬레이션을 기반으로 우리는 다음 항목들을 테스트하는 데 관심이 있습니다\index{testing}.

a) 데이터가 숫자로만 구성되어 있는지 여부.
b) 도시 및 농촌의 합계가 총계 열과 일치하는지 여부.
c) 연령대별 합계가 총계와 일치하는지 여부.
...
이 사례에서 우리는 PDF의 33페이지에 있는 표 2.1에 관심이 있습니다 (@fig-tanzanian-dhs).

![1996년 탄자니아 DHS에서 분석 대상인 표 2.1](figures/tanzanian_dhs.png){#fig-tanzanian-dhs width=95% fig-align="center"}

`stringi` 패키지의 `stri_split_lines()` 함수를 사용하여 해당 페이지의 텍스트를 줄 단위로 분리합니다.
...
먼저 불필요한 설명 텍스트를 제거하고 표 데이터에만 집중합니다. 그런 다음 익숙한 `tidyverse` 접근 방식을 사용할 수 있도록 티블(tibble)로 변환하겠습니다.
...
모든 열이 하나의 문자열로 뭉쳐 있으므로 이를 분리해야 합니다. "Age group"을 "Age-group"으로 변경하여 '공백' 기준으로 나눌 때 단어 자체가 쪼개지지 않도록 방지합니다.
...
이제 행과 열을 정밀하게 다듬어야 합니다. 유용한 팁 중 하나는 **"음의 공간(Negative space)"** 접근법입니다. 즉, 우리가 원하는 데이터(숫자)를 제외한 나머지 "잡음"이 무엇인지 확인하기 위해, 일시적으로 모든 숫자를 제거해 보는 것이죠.
...
마지막으로 **내부 일관성(Internal consistency)**을 점검합니다. 구성 요소(남성, 여성)의 합이 총계와 일치하는지 확인하는 과정입니다.
...
정부 보고서인 PDF 파일은 사람이 읽기에는 좋지만, 기계가 분석하기에는 최악의 형식 중 하나입니다 (@fig-examplekenyancensuspage). 이를 '사각형' 데이터로 변환하는 과정을 살펴보겠습니다.
...
이 예시에서는 나이로비 지역의 PDF 페이지에 초점을 맞추겠습니다 (@fig-examplekenyancensuspage).

![나이로비 지역 2019년 케냐 인구 조사 페이지](figures/09-Kenya_Nairobi.png){#fig-examplekenyancensuspage width=95% fig-align="center"}

#### 데이터 구조 조정 (Reshaping)

PDF에서 읽어온 데이터는 종종 한 줄에 여러 열의 정보가 섞여 있거나 (@fig-examplekenyancensuspage), 보기 좋게 두 개 이상의 표가 나란히 배치되어 있기도 합니다. 이를 우리가 다루기 쉬운 '세로로 긴(Long)' 형태로 합쳐야 합니다.
...
이 데이터셋을 정리하는 원래 목적은 @alexander2021bayesian 에서 사용되는 표를 만드는 것이었습니다. 이 데이터셋으로 다시 돌아오겠지만, 우선 분석 내용을 종합하기 위해 나이로비 지역의 성별·연령별 집계 그래프를 그려보겠습니다 (@fig-monicasnairobigraph).
...
@fig-monicasnairobigraph 에서 연령 집중 현상, 남녀 출생 비율의 미세한 차이, 그리고 15세에서 25세 사이의 뚜렷한 인구 분포 차이 등 다양한 특징을 확인할 수 있습니다.

마지막으로, 더 유익한 이름을 사용하고 싶을 수 있습니다. 예를 들어, 이전 케냐 데이터 예시에서는 "area", "age", "gender", "number"와 같은 열 이름을 사용했죠. 만약 열 이름을 일종의 약속(계약)으로 간주한다면, 데이터 타입을 반영하여 "chr_area", "fctr_group_age", "chr_group_gender", "int_group_count"와 같이 명명할 수도 있습니다.
...
1. *(계획)* 다음 시나리오를 고려해 보세요. "두 명의 직원이 있는 상점을 관리하고 있으며 그들의 업무 효율성을 모델링하려고 합니다. 상점은 오전 9시에 열고 오후 5시에 닫습니다. 직원의 효율성은 서로 어느 정도 상관관계가 있으며, 시간당 응대하는 고객 수로 정의됩니다. 음의 상관관계 또는 양의 상관관계를 가정하는지 명시하세요." 데이터셋의 구조를 스케치한 다음, 전체 관측치를 보여줄 수 있는 그래프를 스케치해 보세요.
2. *(시뮬레이션)* 위 시나리오를 바탕으로 데이터를 시뮬레이션하고, 생성된 데이터셋에 대한 5가지 테스트 코드를 작성하세요. 결과 코드가 포함된 GitHub Gist 링크를 제출하세요.
3. *(수집)* 그러한 데이터셋을 얻을 수 있는 현실적인 출처를 설명해 보세요.
4. *(탐색)* `ggplot2`를 사용하여 1단계에서 스케치한 그래프를 시뮬레이션된 데이터를 활용해 구현해 보세요. 코드가 포함된 GitHub Gist 링크를 제출하세요.
5. *(소통)* 자신이 수행한 작업에 대해 두 단락 내외로 요약해 보세요.

### 퀴즈 {.unnumbered}

1. "some_words"라는 문자 변수에 "You know what"이라는 하나의 관측치가 있는 `sayings`라는 데이터셋이 있다면, 다음 중 그것을 각 단어로 분리해 주는 코드는 무엇입니까?
...
2. 다음은 '정돈된 데이터(Tidy data)'의 사례입니까?

```{r}
#| echo: true
#| eval: false
tibble(
  name = c("Ian", "Patricia", "Bill", "Karen"),
...
```
    ...
    a.  **예**
    b. 아니요
3. "lemons"를 "lemonade"로 변경해 주는 함수는 무엇입니까?
...
4. 연령 데이터를 다룰 때 변수에 대한 바람직한 클래스는 무엇입니까? (모두 선택)
    a.  **정수(Integer)**
    b. 행렬(Matrix)
    c.  **숫자(Numeric)**
5. 다음 독일 도시들을 고려해 보세요: "베를린", "함부르크", "뮌헨", "쾰른", "프랑크푸르트", "로스토크". `testthat`를 사용하여 "german_cities" 변수가 오직 이러한 도시들만 포함하고 있는지 확인하는 세 가지 테스트를 정의하세요. GitHub Gist 링크를 제출하세요.
6. 데이터 과학에서 날짜를 표기하는 가장 적합한 형식은 무엇입니까?
    a. YYYY-DD-MM
    b.  **YYYY-MM-DD**
    c. DD-MM-YYYY
    d. MM-MM-YYYY
7. 다음 중 데이터 분포에서 벗어날 가능성이 가장 높은 값은 무엇입니까? `c(15.9, 14.9, 16.6, 15.8, 16.7, 17.9, I2.6, 11.5, 16.2, 19.5, 15.0)`
8. [[@jsfcodingstandards], p. 25] 의 "AV 규칙 48"과 관련하여, 다음 중 식별자를 서로 다르게 인식할 수 없는 경우는 무엇입니까? (모두 선택)
    a.  **단순한 대소문자 혼합의 차이**
    b.  **밑줄 문자의 유무**
    c.  **문자 "O"와 숫자 "0", 혹은 문자 "D"를 혼용하는 경우**
    d.  **문자 "I"와 숫자 "1", 혹은 문자 "l"을 혼용하는 경우**
9. @Preece1981 장과 관련하여, 숫자의 마지막 자리수가 유익한 정보를 제공할 수 있는 두 가지 사례를 논의해 보세요. 각 사례에 대해 최소 한 단락 이상 작성하고 구체적인 예시를 포함하세요.

### 수업 활동 {.unnumbered}

- 자신이 잘 아는 주제를 하나 선택한 뒤 다음을 수행하세요.
    - R 스크립트로 이상적인 데이터셋을 재현 가능한 방식으로 시뮬레이션하세요.
    - 다른 R 스크립트에서 이를 검증할 수 있는 5가지 테스트 코드를 작성하세요.
    - 데이터셋 생성 코드(테스트 제외)를 GitHub Gist로 공유하세요. 다른 학생이 해당 코드에 의도적으로 3가지 데이터 문제(날짜 형식 오류, 결측치, 음수 값, 소수점 오류 등)를 만들어 다시 보내주면, 작성한 테스트가 이를 성공적으로 식별하는지 확인해 보세요.
- 시뮬레이션 결과를 토대로 다음 주장에 대해 토론해 보세요: "문자열(String)이 요인(Factor)보다 다루기 더 낫다."^[이 연습 과제의 기본 아이디어는 마이클 도넬리(Michael Donnelly)로부터 나왔습니다.]
- 소득 데이터를 얻었는데, 응답자 중 상당수가 "999999"라는 값을 가지고 있습니다. 이것이 왜 우려되는 상황인지 설명해 보세요.
- 새로운 R 프로젝트를 생성하고, R 스크립트를 사용하여 원본 "Adélie 펭귄 데이터" [@penguinsoriginaldata] 를 다운로드하여 로드하세요. (아래의 코드를 참고하세요.^[이 코드는 크리스티나 웨이(Christina Wey)가 작성했습니다.])
    - `rename()`을 사용하여 변수 이름을 개선하세요.
    - 폴더를 GitHub 리포지토리에 추가하세요.
    - 리포지토리를 다른 학생과 교환하여, 이름이 개선될 수 있는 두 가지 지점에 대해 GitHub 이슈를 작성하세요.
    - `palmerpenguins::penguins` 패키지의 데이터와 비교하여 이름이 어떻게 다른지 논의해 보세요.
...
- `anscombe` 데이터셋을 활용해 '앤스콤의 콰르텟'의 정돈된(Tidy) 버전을 생성하세요.
- 미국 일반 사회 조사(US General Social Survey)는 "모름(Don't know)" 응답을 어떻게 코딩하는지 조사해 보세요.
- 결측 데이터를 코딩하는 각 방식의 장단점에 대해 토론해 보세요.
...
- 결측 데이터는 데이터셋에서 어떻게 코딩되는 것이 가장 바람직하다고 생각하시나요?
- 새로운 R 프로젝트를 생성하여 관심 있는 도시에 대해 다음을 수행하세요.^[이 연습 과제의 아이디어는 테일러 존 라이트(Taylor Jon Wright)로부터 나왔습니다.]
    - [ShotSpotter 데이터셋](http://justicetechlab.org/shotspotter-data/)을 다운로드하고, 데이터셋을 정리하는 재현 가능한 코드를 작성하세요.
    - 코드를 GitHub에 추가하세요.
    - 다른 학생과 리포지토리를 교환하여, 상대방의 데이터셋을 활용해 그래프를 그리는 풀 리퀘스트(Pull Request)를 보내보세요.
- ImageNet 데이터셋에서 무작위로 이미지 10개를 열어보세요. 각 이미지에 직접 레이블을 붙여보세요. 자신의 레이블이 실제 데이터셋의 레이블과 어느 정도 일치하나요? 이러한 레이블링의 차이가 모델 훈련 결과에 어떤 영향을 미칠지 토론해 보세요.
- 세대 간의 부의 이동을 연구한다고 가정해 봅시다. 이를 위해 오늘날의 교육 수준 데이터와 100년 전 다른 국가의 교육 데이터를 연결하려고 합니다. 데이터셋을 일관되게 만들고, 그 과정에서 내린 결정 사항들을 문서화하세요.
...
- `datasets::UCBAdmissions` 데이터셋을 활용해 정돈된(Tidy) 데이터를 생성하세요.
- `datasets::LifeCycleSavings` 데이터셋을 활용해 다음을 수행하세요.
    - 먼저 `pivot_longer()`를 사용하여 데이터를 길게 변환하세요.
    - `ggplot`을 사용하여 그래프를 생성하세요.
    - 그런 다음 `pivot_wider()`를 사용하여 데이터를 다시 넓게 변환하세요.
    - 호주, 캐나다, 뉴질랜드 및 자신이 선택한 국가들에 대해 `tinytable::tt()`를 사용하여 요약 표를 만드세요.
- 다음의 데이터셋을 수정해 보세요.
...
- 캐나다 토론토에 살고 있다고 가정해 보세요. 다음의 날짜 정보에 어떤 문제가 있나요?^[이 연습 과제의 아이디어는 데릭 비튼(Derek Beaton)으로부터 나왔습니다.] `2023-03-12 02:01:00`.
- @sec-multilevel-regression-with-post-stratification 절의 예시를 따라 [미국 일반 사회 조사](https://gss.norc.org/Get-The-Data)의 ".dta" 파일을 로드하고, 레이블을 추가한 뒤 일부 변수의 그래프를 그려보세요. 이때 결측 데이터는 어떻게 처리되나요?

### 과제 {.unnumbered}

@Jordan2019Artificial, @datafeminism2020 [6장], [@thatrandyauperson] 및 관련 연구들을 참고하여, 데이터가 스스로 말하도록 어느 정도까지 허용해야 한다고 생각하시나요? 자신의 견해를 담아 최소 두 페이지 분량의 에세이를 작성하세요.

Quarto를 사용하여 제목, 저자, 날짜, GitHub 리포지토리 링크 및 적절한 인용을 포함한 초안을 작성하세요. 작성 후 다른 학생과 과제를 교환하여 피드백을 주고받으세요. 피드백을 바탕으로 내용을 업데이트하고, 감사의 글에 동료의 이름을 명시하세요. 최종 결과물을 PDF로 제출하세요.
