...
구조화된 쿼리 언어(Structured Query Language)의 약자인 SQL("시퀄" 또는 "에스큐엘"이라 읽음)은 관계형 데이터베이스를 관리하고 조작하기 위한 핵심 언어입니다. 관계형 데이터베이스란 하나 이상의 테이블이 서로 유기적으로 연결된 상태를 말하며, 각 테이블은 우리가 이미 익숙한 행과 열의 구조를 가집니다. 데이터베이스 안에 여러 테이블이 존재할 때, 이들을 서로 연결해 주는 기준 열(key)이 반드시 존재해야 합니다. SQL은 우리가 앞서 배운 프로그래밍 언어보다는, 마크업 언어인 HTML이나 CSS를 다루는 것과 다소 유사한 느낌을 줍니다. 한 가지 흥미로운 관습은 예약어(명령어)를 대문자로 작성한다는 점입니다. 또한 줄 바꿈이나 공백은 코드 실행에 영향을 주지 않지만, 모든 SQL 문장은 반드시 세미콜론(`;`)으로 끝맺어야 합니다.

SQL은 1970년대 IBM에서 처음 개발된 이후, 오늘날까지 데이터를 다루는 가장 강력하고 인기 있는 도구로 자리 잡았습니다. 사용 환경에 따라 다양한 "SQL 방언(Dialects)"이 존재하지만, 그 뿌리는 같습니다. 이 장에서는 가볍고 빠르며 Mac에 기본 설치되어 있는 오픈 소스 데이터베이스인 SQLite를 중심으로 설명합니다. Windows 사용자라면 [공식 홈페이지](https://www.sqlite.org/download.html)에서 간편하게 설치할 수 있습니다.

고급 사용자는 SQL만으로도 복잡한 분석 업무를 완결할 수 있지만, 초보자라 하더라도 기본적인 SQL 문법만 익히면 다룰 수 있는 데이터의 폭이 획기적으로 넓어집니다. 특히 기업의 방대한 데이터가 SQL 기반 서버에 저장되어 있는 경우가 많기 때문에, 데이터를 직접 쿼리하여 가져올 수 있는 능력은 업무 효율성 측면에서 커다란 강점이 됩니다.

최근에는 R의 `DBI` [@dbi] 패키지 등을 활용해 RStudio 안에서 직접 SQL을 작성하는 것도 가능해졌습니다. 하지만 R에 대한 숙련도와는 별개로, 노동 시장에서의 SQL 수요는 독립적으로 매우 높기 때문에 RStudio 외부에서 SQL을 다루는 실무 지식을 갖추는 것이 커리어 관점에서 더 유리할 수 있습니다. 사실 우리가 이 책에서 배운 `dplyr`의 동사들과 SQL 명령어들은 개념적으로 매우 닮아 있습니다. R 환경을 벗어나고 싶지 않다면 `dbplyr` [@dbplyr] 패키지를 사용해 보세요. `dplyr` 함수를 사용하면 이를 자동으로 SQL 문으로 변환해 줍니다. 이미 `mutate()`, `filter()`, `left_join()` 등에 익숙하다면 SQL의 핵심 명령어도 금방 배울 수 있을 것입니다. 다만 SQL은 명령어의 작성 순서와 실제 실행 순서가 다를 수 있다는 점에 유의해야 합니다.
...
SQL은 본 도서의 주된 주제는 아니기에 여기서는 필수적인 명령어들만 간략히 살펴보았습니다. 하지만 실무에서 데이터 과학자로 성장하기 위해서는 SQL에 익숙해지는 과정이 반드시 필요합니다. "SQL 없이는 데이터 과학의 여정을 멀리 지속하기 어렵다" [@robinsonnolis2020, p. 8]는 말처럼, 거의 모든 데이터 과학 관련 채용 인터뷰에서는 여러분의 SQL 능력을 검증하려 할 것입니다 [@robinsonnolis2020, p. 110].
