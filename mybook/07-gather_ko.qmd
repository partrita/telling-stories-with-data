...
- *역사를 데이터로 바꾸기: HPE에서의 데이터 수집, 측정 및 추론* 읽기(원제: *Turning History into Data: Data Collection, Measurement, and Inference at the HPE*), [@cirone]
  - 이 논문은 데이터셋 생성의 몇 가지 어려움에 대해 논의합니다.
- *박스 스코어 데이터 수집의 두 가지 체제* 읽기(원제: *Two Regimes of Box-Score Data Collection*), [@Johnson2021Two]
  - 이 논문은 두 가지 다른 시대의 NBA 박스 스코어 데이터를 비교합니다.
- *AI 아틀라스* 읽기(원제: *The Atlas of AI*), [@crawford]
...
이 장에서는 우리가 직접 수집해야 하는 데이터를 다룹니다. 이는 관측치가 존재하지만, 우리가 고려할 데이터셋을 얻기 위해 그것들을 파싱하고, 가져오고, 정리하고, 준비해야 한다는 것을 의미합니다. [@sec-farm-data]에서 논의된 경작 데이터와 달리, 종종 이러한 관측치는 분석 목적으로 제공되지 않습니다. 이는 문서화, 포함 및 제외 결정, 결측 데이터, 윤리적 행동에 특히 주의해야 함을 의미합니다.

이러한 데이터셋의 예로, @Cummins2022 1892년에서 1992년 사이에 영국에서 개별 유언 검인 기록을 사용하여 데이터셋을 생성했습니다. 그들은 "엘리트" 상속의 약 3분의 1이 은폐되어 있음을 발견했습니다.\index{England!inheritance} 마찬가지로, @Taflaga2019 호주 장관 전화번호부를 기반으로 직무 책임에 대한 체계적인 데이터셋을 구축했습니다. 그들은 성별에 따라 상당한 차이가 있음을 발견했습니다. 유언장이나 전화번호부 모두 데이터셋에 포함될 목적으로 생성된 것이 아닙니다. 그러나 존중하는 접근 방식을 통해 다른 방법으로는 얻을 수 없는 통찰력을 얻을 수 있습니다. 우리는 이것을 "데이터 수집"이라고 부릅니다. 즉, 데이터는 존재하지만 우리는 그것을 얻어야 합니다.
...
데이터 수집은 경작 데이터를 사용하는 것보다 더 많은 노력을 요구할 수 있지만, 다른 방법으로는 할 수 없는 데이터셋을 탐색하고 질문에 답할 수 있게 해줍니다. 세계에서 가장 흥미로운 작업 중 일부는 수집된 데이터를 사용하지만, 존중하는 태도로 접근하는 것이 특히 중요합니다.
...
`httr`를 설치하고 로드한 후 `GET()`을 사용하여 API에서 직접 데이터를 가져옵니다. 이것은 특정 데이터를 가져오려고 시도하며 주요 인수는 "url"입니다. 이것은 @fig-focuson2020 Google Maps 예시와 유사합니다. 여기서 우리가 관심 있는 특정 정보는 지도였습니다.
...
노래를 기반으로 다양한 정보를 사용할 수 있습니다. 시간이 지남에 따라 노래가 길어지는지 여부에 관심이 있을 수 있습니다(@fig-readioovertime). @sec-static-communication 지침에 따라 이것은 앨범별 요약 통계를 동시에 전달하기 위해 상자 그림을 추가로 사용할 수 있는 좋은 기회입니다.
...
이 모든 정보가 아주 적은 노력과 비용으로 제공되는 세상에 살고 있다는 것이 얼마나 놀라운 일입니까! 그리고 데이터를 수집한 후에는 할 수 있는 일이 많습니다. 예를 들어, @kaylinpavlik 는 확장된 데이터셋을 사용하여 음악 장르를 분류하고 @theeconomistonspotify 는 언어가 Spotify에서 음악 스트리밍과 어떻게 관련되어 있는지 살펴봅니다. 이러한 데이터를 수집할 수 있는 능력은 과거에는 실험적으로 고려해야 했던 질문에 답할 수 있게 해줍니다. 예를 들어, @salganik2006experimental 이제 접근할 수 있는 관찰 데이터 대신 실험 데이터를 사용하여 히트곡을 만드는 사회적 측면을 분석해야 했습니다.
...
텍스트를 데이터로 간주하는 것은 흥미롭고 다양한 연구 질문을 탐색할 수 있게 해줍니다.\index{text!gathering} @sec-text-as-data 이를 활용할 것입니다. 많은 가이드에서는 이미 잘 형식화된 텍스트 데이터셋이 있다고 가정하지만, 실제로는 거의 그렇지 않습니다. 이 사례 연구에서는 몇 개의 다른 페이지에서 파일을 다운로드할 것입니다. 웹 스크래핑의 두 가지 예시를 이미 보았지만, 그것들은 한 페이지에만 초점을 맞추었지만, 우리는 종종 많은 페이지가 필요합니다. 여기서는 이 반복에 초점을 맞출 것입니다. `download.file()`를 사용하여 다운로드를 수행하고, `purrr`를 사용하여 여러 사이트에 이 함수를 적용할 것입니다.\index{web scraping!multiple files} `purrr`는 핵심 `tidyverse`의 일부이므로 설치하거나 로드할 필요가 없습니다. `tidyverse`를 로드하면 로드됩니다.
...
그 결과, 이 PDF들을 다운로드하여 컴퓨터에 저장했습니다. 이러한 함수를 직접 작성하는 대신 `heapsofpapers`[@heapsofpapers]를 사용할 수도 있습니다. 여기에는 특히 PDF, CSV, txt 파일과 같은 파일 목록을 다운로드하는 데 유용한 다양한 옵션이 포함되어 있습니다. 예를 들어, @collinsalexander 이를 사용하여 수천 개의 PDF를 얻고 COVID-19 연구가 재현 가능한 정도를 추정합니다.
...
일반적으로 결과는 나쁘지 않습니다. OCR은 유용한 도구이지만 완벽하지 않으며 결과 데이터는 정리 측면에서 추가적인 주의가 필요할 수 있습니다. 예를 들어, @fig-janescan OCR 결과에서 수정해야 할 불규칙성을 볼 수 있습니다. 관심 있는 특정 데이터에 초점을 맞추고 대비를 높이는 것과 같은 다양한 옵션이 도움이 될 수 있습니다. 다른 인기 있는 OCR 엔진으로는 Amazon Textract, Google Vision API 및 ABBYY가 있습니다.\index{text!gathering}
...
4. UN의 [데이터 API](https://population.un.org/dataportal/about/dataapi)와 @schmertmannunapi 사용법 소개를 고려하십시오. 아르헨티나의 위치 코드는 32입니다. 다음 코드를 수정하여 1995년 20세 아르헨티나의 단일 연령 출산율이 얼마였는지 확인하십시오 (하나 선택)?
    a. 147.679
    b. 172.988
    c. 204.124
    d.  128.665
...
18. 스캔한 문서에 OCR을 수행할 때 텍스트 인식의 정확도에 영향을 미칠 수 있는 일반적인 문제는 무엇입니까 (하나 선택)?
    ...
    c.  스캔한 이미지의 품질 및 해상도.
    d. 문서의 페이지 수.
19. @cirone 따르면, 다음 중 역사적 데이터로 작업할 때 추론에 대한 일반적인 위협이 아닌 것은 무엇입니까 (하나 선택)?
    a. 선택 편향.
    b. 확증 편향.
    c. 시간 경과.
    d.  소외된 그룹의 과대 대표.
20. @cirone 따르면, 역사적 정치 경제학(및 더 일반적으로)에서 술꾼의 검색 문제란 무엇입니까 (하나 선택)?
...
21. @cirone 따르면, DAG는 어떤 역할을 합니까 (하나 선택)?
...
22. @Johnson2021Two 따르면, 미국 인구 조사국의 초기 교도소 데이터 수집의 초점은 무엇이었습니까 (하나 선택)?
    a. 건강 상태 문서화.
    b. 선고에서 인종적 차이 조사.
    c. 사회경제적 배경 및 고용 기록.
    d.  수감자 수 및 인구 통계 집계.
23. @Johnson2021Two 따르면, 지역 사회에서 수집된 교도소 데이터는 주에서 수집된 교도소 데이터와 어떻게 다릅니까 (하나 선택)?
    a. 지역 사회 데이터는 정부 공무원이 수집합니다.
    b.  지역 사회 데이터는 경험과 교도소 조건을 강조합니다.
    c. 주 데이터는 지역 사회 데이터보다 신뢰성이 낮습니다.
    d. 주 데이터는 지역 사회 데이터보다 신뢰성이 높습니다.
24. @Johnson2021Two 따르면, 다음 중 주에서 수집된 데이터의 한계는 무엇입니까 (하나 선택)?
    a. 주에서 수집된 데이터는 학술 연구보다 신뢰성이 낮습니다.
    b. 교도소 인구를 과소 대표합니다.
    c.  이전 데이터 수집의 편향과 가정을 재현할 수 있습니다.
    d. 비폭력 범죄자에게만 초점을 맞춥니다.
25. @Johnson2021Two 따르면, 교도소 데이터 수집을 볼 때 어떤 질문을 해야 합니까 (하나 선택)?
...
- @fig-pmslives 동등한 것을 캐나다에 대해 만드십시오.
- *논문 검토:* @Kish1959 읽고 익숙한 예를 들어 최소 한 페이지의 검토를 작성하십시오.
