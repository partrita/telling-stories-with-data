...
- MRP의 근본적인 가정은 성별, 연령대, 거주 지역 등 예측 변수와 결과(예: "누구에게 투표할 것인가?") 사이의 관계가 설문 표본과 사후 층화 데이터셋(모집단 데이터) 사이에서 안정적으로 유지된다는 점입니다. MRP 추정 결과의 신뢰도를 판단할 때 가장 핵심적인 질문은 "이 가정이 얼마나 설득력 있는가?"입니다.
...
현실적으로 비확률 표본(편의 추출 등)은 수집 비용이 저렴하므로 이를 유용하게 활용할 수 있다는 점은 큰 매력입니다. 하지만 명심해야 할 것은 MRP가 모든 문제를 해결하는 '마법의 지팡이'는 아니라는 점입니다.\index{statistics}\index{multilevel regression with post-stratification!not magic bullet} 확률 표본을 직접 사용할 때보다 불확실성(표준 오차)은 당연히 더 커지며, 데이터 자체가 가진 원천적인 편향에서 완전히 자유로울 수는 없습니다.
...
MRP는 강력하지만 고려해야 할 트레이드오프도 분명합니다.\index{multilevel regression with post-stratification!trade-offs} 예를 들어 @si2020use 등은 어떤 모델링 접근 방식이 가장 적절한지에 대해 심도 있게 논의했습니다. 또한 @lauderdale2020model 이나 @ghitza2020voter 등은 설문 설계의 불확실성이 최종 결과에 미치는 영향을 연구했습니다.
...
그룹별 인구 비중이 대략 비슷하게 구성된 것을 확인할 수 있습니다(@tbl-teapreferencecounts).
...
이제 이 편향된 설문 데이터를 기반으로 모델을 '훈련'시킵니다. 연령과 국적을 독립 변수로 하여 차 선호도를 예측하는 모델을 구축합니다. 여기서는 각 하위 그룹의 효과를 정교하게 추정하기 위해 **다단계 모델(Multilevel Model)**을 사용합니다. 이는 모든 데이터를 하나로 뭉쳐서 분석하는 대신, 연령과 국적이라는 그룹 구조를 명시적으로 고려하여 정보를 효율적으로 활용하게 해줍니다.
...
여기서 $y_i$는 바이든을 지지할 경우 1, 그렇지 않을 경우 0의 값을 가집니다. 각 $\alpha$ 항들은 각 변수가 투표 확률에 미치는 효과를 잡아냅니다. 특히 주와 교육 수준처럼 범주가 많은 변수들을 무작위 효과(Random effects)로 처리함으로써, 표본 수가 적은 주에서도 전국적인 트렌드로부터 정보를 '빌려와' 안정적인 추정을 가능하게 합니다.
...
이제 앞서 계산한 모집단 비율 데이터셋을 활용해 사후 층화를 수행합니다. 이 과정을 통해 각 주별로 가중치가 적용된 최종 지지율 추정치와 그에 따른 95% 신뢰 구간을 도출할 수 있습니다.
...
1. MRP(사후 층화 다단계 회귀)가 무엇인지 정의하고, 이 분석 기법에서 사용되는 주요 전문 용어들을 포함하여 장단점을 상세히 설명해 보세요. (최소 세 단락 이상)
