...
@fig-teamodelresultsplots 각기 다른 그룹에 대한 표본 분포를 보여줍니다.
...
#| fig-cap: "연령 및 국적별 차 선호도에 대한 다단계 모델의 표본 분포 검토"
...
@tbl-mrpteammodelcompare MRP 추정치를 편향된 표본의 원시 추정치와 비교합니다. 이 경우, 우리는 진실을 알고 있으므로 알려진 진실과도 비교할 수 있지만, 일반적으로는 그렇게 할 수 없습니다.
...
@sec-farm-data 수집한 2019년 미국 지역사회 조사(ACS)\index{American Community Survey} 데이터셋을 사후 층화 데이터셋으로 사용할 것입니다.
...
이전에 정리한 데이터를 읽어들인 후, @kennedygabry2020 따라 `rstanarm`의 `stan_glmer()`를 사용하여 모델을 추정합니다.
...
@fig-politicsmodelresultsplots 연령대 및 교육에 대한 표본 분포를 보여줍니다. 공간상의 이유로 일부 선택된 주를 별도로 플로팅합니다(@fig-politicsmodelresultsplotsstates).
...
#| fig-cap: "각 주에 대한 MRP 사후 추정치와 Nationscape 원시 데이터 비교"
#| message: false
#| fig-height: 8
